...
response_phase1 <- phase1 %>%
  group_by (pNum, block_real) %>%
  summarise(mean_response = mean(prob_response, na.rm = TRUE))
#now set block as a factor
response_phase1$block_real <- factor(response_phase1$block_real)
response_phase1$pNum <- factor(response_phase1$pNum)
#now one between subjects ANOVA
ANOVA_p1_resp <-  aov_car(formula = mean_response ~ Error(pNum/block_real), data = response_phase1)
bay_ANOVA_p1_resp <- anovaBF(formula = mean_response ~ block_real + pNum,
...
response_phase2 <- phase2 %>%
  group_by (pNum, block_real) %>%
  summarise(mean_response = mean(prob_response, na.rm = TRUE))
response_phase2$block_real <- factor(response_phase2$block_real)
response_phase2$pNum <- factor(response_phase2$pNum)
ANOVA_p2_resp <- aov_car(formula = mean_response ~ Error(pNum/block_real), data = response_phase2)
bay_ANOVA_p2_resp <- anovaBF(formula = mean_response ~ block_real + pNum,
